...
def create_team(community):
	...
	forward = Player.query.filter_by(community_id = community.id, username = forward).first()
	if forward is None:
	...
	goalkeeper = Player.query.filter_by(community_id = community.id, username = goalkeeper).first()
...
def create_match(community):
	...
	team0 = Team.query.filter_by(community_id = community.id, name = teams[0]).first()
	if team0 is None:
		return jsonify(message = "teams[0]: not found"), 400

	team1 = Team.query.filter_by(community_id = community.id, name = teams[1]).first()
	if team1 is None:
